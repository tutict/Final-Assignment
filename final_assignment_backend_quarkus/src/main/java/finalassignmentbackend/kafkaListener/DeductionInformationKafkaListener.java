...
public class DeductionInformationKafkaListener {
    ...
    // 监听"deduction_create"主题的消息，处理扣押信息创建
    ...
    // 监听"deduction_update"主题的消息，处理扣押信息更新
    ...
    private void processMessage(String message, String action, MessageProcessor<DeductionInformation> processor) {
        try {
            ...
            log.log(Level.INFO, "反序列化扣押信息对象: {0}", deductionInformation);
            // 对于创建操作，重置扣押ID
            ...
            log.info(String.format("扣押%s操作处理成功: %s", action, deductionInformation));
        } catch (Exception e) {
            // 记录处理错误日志
            log.log(Level.SEVERE, String.format("处理%s扣押消息时出错: %s", action, message), e);
            throw new RuntimeException(String.format("无法处理%s扣押消息", action), e);
        }
    }
