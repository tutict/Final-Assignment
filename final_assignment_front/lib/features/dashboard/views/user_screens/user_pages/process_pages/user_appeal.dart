...
class _UserAppealPageState extends State<UserAppealPage> {
  ...
  Future<void> _fetchAllAppeals() async {
    ...
    try {
      ...
      // 返回类型: Future<List<Object>?> ，可能为 null
      ...
      // 将 List<Object> -> List<AppealManagement>
      final appeals = listObj.map((item) =>
  ...
  Future<void> _searchAppealsByName(String name) async {
    ...
    try {
      ...
      final result = await appealApi.apiAppealsNameAppealNameGet(appealName: name);
      // 这个方法返回的是 Object? 可能是单个 or List
      // 需要看后端具体实现
      // 如果后端返回列表, 你可以把它当成 List<Object>.
      ...
      if (result is Map<String, dynamic>) {
        ...
        // 如果后端返回列表
        final appeals = result.map((item) => AppealManagement.fromJson(item)).toList();
        setState(() {
          _appeals = appeals;
          _isLoading = false;
        });
  ...
  Future<void> _createAppeal(AppealManagement appeal) async {
    try {
      final result = await appealApi.apiAppealsPost(appealManagement: appeal);
      // 创建成功后刷新列表
      await _fetchAllAppeals();
    } catch (e) {
  ...
  Future<void> _deleteAppeal(int appealId) async {
    try {
      final result = await appealApi.apiAppealsAppealIdDelete(
          ...
          ElevatedButton(
            onPressed: () {
              final newAppeal = AppealManagement()
                ..appellantName = nameController.text
                ..appealReason = reasonController.text
                ..processStatus = '处理中'; // 仅示例
