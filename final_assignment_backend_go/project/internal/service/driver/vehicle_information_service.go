...
func (s *VehicleService) GetLicensePlateAutocomplete(idCard string, prefix string, max int) []string {
	query := s.DB().Table("vehicle_information").Select("DISTINCT license_plate").Where("license_plate LIKE ?", shared.PrefixLike(prefix))
	if idCard != "" {
...
func (s *VehicleService) GetVehicleTypeAutocomplete(idCard string, prefix string, max int) []string {
	query := s.DB().Table("vehicle_information").Select("DISTINCT vehicle_type").Where("vehicle_type LIKE ?", shared.PrefixLike(prefix))
	if idCard != "" {
		query = query.Where("owner_id_card = ?", idCard)
	}
	var values []string
...
func (s *VehicleService) GetLicensePlateGlobally(prefix string) []string {
	return shared.DistinctStrings(s.DB(), "vehicle_information", "license_plate", prefix, 10)
}

func (s *VehicleService) GetVehicleTypeGlobally(prefix string) []string {
	return shared.DistinctStrings(s.DB(), "vehicle_information", "vehicle_type", prefix, 10)
}
...
func (s *VehicleService) GetByOwnerName(ownerName string) []domain.VehicleInformation {
	var vehicles []domain.VehicleInformation
	_ = s.DB().Where("owner_name LIKE ?", shared.Like(ownerName)).Find(&vehicles).Error
	return vehicles
}
...
func (s *VehicleService) UpdateVehicle(key string, vehicle *domain.VehicleInformation) error {
	if err := shared.CheckIdempotency(key, "vehicle:update"); err != nil {
		return err
	}
