...
func (s *PermissionManagementService) CheckAndInsertIdempotency(key string, permission *domain.PermissionManagement, operation string) error {
	if err := shared.CheckIdempotency(key, "permission:"+operation); err != nil {
		return err
	}
	now := time.Now()
	if strings.EqualFold(operation, "create") {
		...
		permission.CreatedTime = shared.TimePtr(now)
		permission.ModifiedTime = shared.TimePtr(now)
...
func (s *PermissionManagementService) GetPermissionsByNameLike(name string) ([]domain.PermissionManagement, error) {
	...
	err := s.DB().Where("permission_name LIKE ?", shared.Like(name)).Find(&permissions).Error
...
type PermissionService = PermissionManagementService
