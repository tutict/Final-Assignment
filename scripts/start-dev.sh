...
Optional environment variables:
  ...
  APP_DOCKER_STARTUP_SCRIPT_ENABLED  Starts local Docker Desktop/Engine and compose services before Spring context. Default: true in dev profile
  APP_OLLAMA_STARTUP_SCRIPT_ENABLED  Starts local Ollama before Spring context. Default: true in dev profile
  ...
  FLUTTER_DEVICE        Flutter device id, such as linux, chrome, android.
  FLUTTER_ARGS          Extra flutter run arguments.
EOF
}
...
(
  ...
  mvn spring-boot:run "-Dspring-boot.run.profiles=$BACKEND_PROFILE" "-Dspring-boot.run.jvmArguments=-Dspring.devtools.restart.enabled=false" ${BACKEND_ARGS:-}
