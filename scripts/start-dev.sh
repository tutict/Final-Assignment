...
Optional environment variables:
  ...
  REACT_FALLBACK_PORTS         React dev-server fallback ports. Default: 15173 4173 51730
  ...
  FLUTTER_FALLBACK_PORTS       Flutter web fallback ports. Default: 13000 3001 24678
...
if [ -n "${WS_BASE_URL:-}" ]; then
  WS_BASE_URL_EXPLICIT="true"
else
  WS_BASE_URL_EXPLICIT="false"
fi
...
if [ -n "${FLUTTER_WEB_URL:-}" ]; then
  FLUTTER_WEB_URL_EXPLICIT="true"
else
  FLUTTER_WEB_URL_EXPLICIT="false"
fi
...
NPM_CMD="${NPM_CMD:-npm}"
if [ -n "${REACT_DEV_URL:-}" ]; then
  REACT_DEV_URL_EXPLICIT="true"
else
  REACT_DEV_URL_EXPLICIT="false"
fi
...
if [ -n "${FRONTEND_URL:-}" ]; then
  FRONTEND_URL_EXPLICIT="true"
else
  FRONTEND_URL_EXPLICIT="false"
fi
if [ -n "${FLUTTER_URL:-}" ]; then
  FLUTTER_URL_EXPLICIT="true"
else
  FLUTTER_URL_EXPLICIT="false"
fi
...
REACT_DEV_PORT_NOTICE=""
FLUTTER_WEB_PORT_NOTICE=""

if [ "$SKIP_ENV" = "true" ]; then
  START_LOCAL_SERVICES="false"
fi
if [ "$START_LOCAL_SERVICES" != "true" ] && [ -z "${MANAGEMENT_HEALTH_REDIS_ENABLED:-}" ]; then
  MANAGEMENT_HEALTH_REDIS_ENABLED="false"
fi
export MANAGEMENT_HEALTH_REDIS_ENABLED
...
update_frontend_routing_defaults() {
  ...
  default_flutter_api_base_url="http://localhost:$BACKEND_PORT"
  default_flutter_ws_base_url="ws://localhost:$BACKEND_PORT"
  ...
  if [ "$API_BASE_URL_EXPLICIT" != "true" ]; then
    API_BASE_URL="$default_flutter_api_base_url"
  fi
  if [ "$WS_BASE_URL_EXPLICIT" != "true" ]; then
    WS_BASE_URL="$default_flutter_ws_base_url"
  fi
  FRONTEND_ORIGIN=""
  case "$FRONTEND_CHOICE" in
    react)
    ...
    flutter)
      ...
      if [ "$FLUTTER_URL_EXPLICIT" != "true" ]; then
        FLUTTER_URL="$FLUTTER_WEB_URL"
      fi
      ;;
  esac
  if [ "$FRONTEND_URL_EXPLICIT" != "true" ] && [ -n "$FRONTEND_ORIGIN" ]; then
    FRONTEND_URL="$FRONTEND_ORIGIN"
  fi
  if [ "$BROWSER_URL_EXPLICIT" != "true" ] && [ -n "$FRONTEND_ORIGIN" ]; then
    BROWSER_URL="$FRONTEND_ORIGIN"
  fi
  export FRONTEND_URL FLUTTER_URL BROWSER_URL REACT_API_BASE_URL REACT_WS_BASE_URL API_BASE_URL WS_BASE_URL REACT_DEV_URL REDPANDA_KAFKA_HOST_PORT BACKEND_PORT BACKEND_INTERNAL_PORT BACKEND_HEALTH_URL
}
...
print_ports() {
  ...
  if command -v lsof >/dev/null 2>&1; then
    react_port=$(printf '%s' "$REACT_DEV_URL" | awk -F: '{print $NF}' | tr -cd '0-9')
    lsof -nP -iTCP:"$BACKEND_PORT" -iTCP:"$BACKEND_INTERNAL_PORT" -iTCP:3000 -iTCP:"${react_port:-5173}" -sTCP:LISTEN >&2 || true
...
set_react_dev_url() {
  host="$(react_dev_host)"
  REACT_DEV_URL="http://$host:$1"
  export REACT_DEV_URL
}


flutter_web_host() {
  hostport=${FLUTTER_WEB_URL#*://}
  hostport=${hostport%%/*}
  host=${hostport%%:*}
  printf '%s' "${host:-127.0.0.1}"
}
...
set_flutter_web_port() {
  host="$(flutter_web_host)"
  FLUTTER_WEB_URL="http://$host:$1"
  export FLUTTER_WEB_URL
  case " $FLUTTER_ARGS " in
    *" --web-port "*)
      FLUTTER_ARGS="$(printf '%s' "$FLUTTER_ARGS" | sed "s/--web-port[[:space:]][0-9][0-9]*/--web-port $1/")"
      ;;
    *)
      FLUTTER_ARGS="$FLUTTER_ARGS --web-port $1"
      ;;
  esac
  export FLUTTER_ARGS
}

resolve_flutter_web_port() {
  [ "$FRONTEND_CHOICE" = "flutter" ] || return 0
  [ "$FLUTTER_WEB_URL_EXPLICIT" != "true" ] || return 0
  initial_port="$(flutter_web_port)"
  case "$FLUTTER_ARGS" in
    *--web-port*)
      arg_port=$(printf '%s' "$FLUTTER_ARGS" | sed -n 's/.*--web-port[[:space:]]\([0-9][0-9]*\).*/\1/p')
      [ -n "$arg_port" ] && initial_port="$arg_port"
      ;;
  esac
  if port_bindable "$initial_port"; then
    set_flutter_web_port "$initial_port"
    return 0
  fi
  listeners="$(describe_port_listeners "$initial_port")"
  [ -z "$listeners" ] || return 0
  for candidate in $(printf '%s' "$FLUTTER_FALLBACK_PORTS" | tr ',;' '  '); do
    case "$candidate" in
      ''|*[!0-9]*) continue ;;
    esac
    if port_bindable "$candidate"; then
      set_flutter_web_port "$candidate"
      FLUTTER_WEB_PORT_NOTICE="Flutter web port $initial_port is not bindable and has no listening process; using $(flutter_web_host):$candidate instead."
      return 0
    fi
  done
  fail "Flutter web port $initial_port is not available, and no fallback ports are bindable. Set FLUTTER_WEB_URL/FLUTTER_ARGS or FLUTTER_FALLBACK_PORTS."
}

smoke_login() {
  [ "$SMOKE_LOGIN" = "true" ] || return 0
  [ "$BACKEND_CHOICE" != "none" ] || return 0
  ...
  if [ "$username" = "admin" ] && [ "$password" = "admin123" ]; then
    username="smoke-${STARTUP_RUN_ID}@local.test"
    password="pass12345"
    log "Registering login-smoke user $username..."
    key="$(date +%s)-$$"
    curl -sS --max-time 15 -H "Content-Type: application/json" -d "$(printf '{"username":"%s","password":"%s","idempotencyKey":"%s"}' "$username" "$password" "$key")" "$base/api/auth/register" >/dev/null 2>&1 || true
  fi
  login_url="$base/api/auth/login"
  log "Running login smoke against $login_url as $username..."
  body=$(printf '{"username":"%s","password":"%s"}' "$username" "$password")
  if command -v curl >/dev/null 2>&1; then
    resp=$(curl -sS --max-time 15 -H "Content-Type: application/json" -d "$body" "$login_url") || fail "Login smoke failed against $login_url"
  else
    fail "Login smoke requires curl."
  fi
  printf '%s' "$resp" | grep -E '"jwtToken"|"accessToken"' >/dev/null 2>&1 || fail "Login smoke succeeded HTTP-wise but response had no jwtToken/accessToken: $resp"
  log "Login smoke succeeded. Token fields present for $username."
}

resolve_react_dev_port() {
  [ "$FRONTEND_CHOICE" = "react" ] || return 0
  [ "$REACT_DEV_URL_EXPLICIT" != "true" ] || return 0

  initial_port="$(react_dev_port)"
  if port_bindable "$initial_port"; then
    return 0
  fi
  listeners="$(describe_port_listeners "$initial_port")"
  ...
  for candidate in $(printf '%s' "$REACT_FALLBACK_PORTS" | tr ',;' '  '); do
    case "$candidate" in
      ''|*[!0-9]*) continue ;;
    esac
    if port_bindable "$candidate"; then
      set_react_dev_url "$candidate"
      REACT_DEV_PORT_NOTICE="React dev port $initial_port is not bindable and has no listening process; using $(react_dev_host):$candidate instead."
      return 0
    fi
  done
  fail "React dev port $initial_port is not available, and no fallback ports are bindable. Set REACT_DEV_URL to a free port, or adjust REACT_FALLBACK_PORTS."
}
...
effective_backend_health_url() {
  ...
  case "$BACKEND_CHOICE" in
    ...
    quarkus) printf 'http://127.0.0.1:%s/readyz\n' "$BACKEND_PORT" ;;
    cloud) printf 'http://127.0.0.1:%s/actuator/health\n' "$BACKEND_PORT" ;;
...
REACT_DEV_URL_EXPLICIT=$REACT_DEV_URL_EXPLICIT
REACT_FALLBACK_PORTS=$REACT_FALLBACK_PORTS
...
if [ -n "${REACT_DEV_PORT_NOTICE:-}" ]; then
  log "$REACT_DEV_PORT_NOTICE"
fi
if [ -n "${FLUTTER_WEB_PORT_NOTICE:-}" ]; then
  log "$FLUTTER_WEB_PORT_NOTICE"
fi
  ...
  case "$BACKEND_CHOICE" in
    spring)
      ...
      (
        ...
        export MANAGEMENT_HEALTH_ELASTICSEARCH_ENABLED MANAGEMENT_HEALTH_REDIS_ENABLED SPRING_AI_OLLAMA_INIT_PULL_MODEL_STRATEGY
    ...
    quarkus)
      ...
      (
        ...
        export QUARKUS_DATASOURCE_JDBC_URL="jdbc:mysql://localhost:3306/traffic?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true"
...
start_frontend() {
  case "$FRONTEND_CHOICE" in
    ...
    react)
      ...
      log "React dev server: $(react_dev_host):$(react_dev_port)"
      ...
      (
        ...
        # shellcheck disable=SC2086
        "$NPM_CMD" run dev -- --host "$(react_dev_host)" --port "$(react_dev_port)" --strictPort ${REACT_ARGS:-}
      ) >"$FRONTEND_LOG" 2>"$FRONTEND_ERR_LOG" &
      FRONTEND_PID=$!
      ;;
    none)
